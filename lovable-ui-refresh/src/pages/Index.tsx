...
const Index = () => {
  ...
  const handleDiscover = async (genre: string) => {
      ...
      try {
        ...
        const response = await fetch(`http://127.0.0.1:8000/discover?${params}`);
  ...
  const handleSearchSubmit = async (overrideQuery?: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/recommend?${params}`);
  ...
  return (
    <div className="min-h-screen bg-background relative" onKeyDown={handleKeyDown}>
